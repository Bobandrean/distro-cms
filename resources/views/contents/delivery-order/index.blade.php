                            ...
                            <div class="table-responsive">
                                <form action="" method="GET" enctype="multipart/form-data">
                                    <table class="table table-bordered table-striped">
                                        ...
                                        <tr>
                                            <th>@lang('pages/delivery-order.table.col_1')</th>
                                            <th>@lang('pages/delivery-order.table.col_2')</th>
                                            <th>@lang('pages/delivery-order.table.col_3')</th>
                                            <th>@lang('pages/delivery-order.table.col_4')</th>
                                            <th>@lang('pages/delivery-order.table.col_5')</th>
                                            <th>@lang('pages/delivery-order.table.col_6')</th>
                                            <th>@lang('pages/delivery-order.table.col_7')</th>
                                            <th>@lang('pages/delivery-order.table.col_8')</th>
                                        </tr>
                                        <tr>
                                            <th>
                                            ...
                                            <th>
                                                <select name="status" class="form-control form-control-sm">
                                                    <option value="">@lang('global.all_selectbox_txt')</option>
                                                    <option value="Diproses" @if(isset($request->status) && $request->status == 'Diproses') selected @endif>Diproses</option>
                                                    <option value="Dalam Pengiriman" @if(isset($request->status) && $request->status == 'Dalam Pengiriman') selected @endif>Dalam Pengiriman</option>
                                                </select>
                                            </th>
                                        ...
                                        @foreach($items as $item)
                                            <tr>
                                                ...
                                                <td>{{ $item->po->pembeli->nama_usaha }} <br> ({{ $item->po->pembeli->nama_depan }} {{ $item->po->pembeli->nama_belakang }})</td>
                                                ...
                                                @elseif($item->status_do == 'Diproses')
                                                    <td class="text-dark">{{$item->status_do}}</td>
                                                @endif
                                                @if($item->foto_bukti_pengiriman == NULL)
                                                    <td class="text-danger">-</td>
                                                @else
                                                    <td><a href="{{ $item->foto_bukti_pengiriman}}" target="_blank">Lihat Berkas</a>
                                                    </td>
                                                @endif
                                                @if($item->foto_invoice == NULL)
                                                    <td class="text-danger">-</td>
                                                @else
                                                    <td><a href="{{ $item->foto_invoice }}" target="_blank">Lihat Berkas</a>
                                                    </td>
                                                @endif
                                                <td></td>
                                            </tr>
